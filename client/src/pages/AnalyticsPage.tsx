          ...
          <section className="rounded-md border border-ink-100 bg-ink-0 overflow-x-auto">
            ...
            <table className="w-full text-sm min-w-[760px]">
              ...
              <tbody className="divide-y divide-ink-100">
                {rep.byCategory.map((c) => (
                  <tr key={c.categoryId}>
                    <td className="px-4 py-2 font-medium text-ink-900">{c.categoryName}</td>
                    <td className="px-4 py-2 text-xs text-ink-600">{c.metalType}</td>
                    <td className="px-4 py-2 text-right tabular-nums">{c.count}</td>
                    <td className="px-4 py-2 text-right"><Weight mg={c.weightMg} /></td>
                    <td className="px-4 py-2 text-right"><Money paise={c.costPaise} /></td>
                    <td className="px-4 py-2 text-right text-ink-900 font-medium">
                      <Money paise={c.marketPaise} />
                    </td>
                    <td className={cn('px-4 py-2 text-right font-semibold', c.unrealizedProfitPaise >= 0 ? 'text-success-700' : 'text-danger-700')}>
                      <Money paise={c.unrealizedProfitPaise} />
                    </td>
                  </tr>
                ))}
              </tbody>
