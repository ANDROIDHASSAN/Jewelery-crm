  ...
  return (
    <>
      ...
      <section className="relative isolate overflow-hidden bg-ink-900">
        ...
        />
        <div className="relative max-w-[1280px] mx-auto px-4 sm:px-6 pt-20 pb-24 sm:pt-28 sm:pb-32 md:pt-44 md:pb-48 text-ink-0">
          <p className="text-eyebrow uppercase text-brand-200">{hero.eyebrow}</p>
          <h1 className="font-display text-[32px] leading-[1.06] sm:text-[44px] sm:leading-[1.04] md:text-[64px] lg:text-[88px] md:leading-[1] tracking-tight max-w-3xl mt-4 sm:mt-5">
            {hero.title}
          </h1>
          <p className="mt-4 sm:mt-6 max-w-xl text-sm sm:text-base md:text-lg text-ink-100/90 leading-relaxed">
            {hero.subtitle}
          </p>
          <div className="mt-6 sm:mt-10 flex flex-wrap items-center gap-3">
            <Link
              to={hero.ctaHref}
              className="inline-flex items-center gap-2 h-11 sm:h-12 px-5 sm:px-7 rounded-full bg-brand-400 text-ink-900 text-sm font-medium hover:bg-brand-300 transition-colors duration-fast"
            >
              {hero.ctaLabel}
            ...
            {hero.secondaryCtaLabel && (
              <Link
                ...
                className="inline-flex items-center gap-2 h-11 sm:h-12 px-5 sm:px-6 rounded-full border border-ink-0/30 text-ink-0 text-sm hover:bg-ink-0/10 transition-colors duration-fast backdrop-blur-sm"
              >
                {hero.secondaryCtaLabel}
              </Link>
            )}
        ...
        <div className="relative border-t border-ink-0/10 bg-ink-900/60 backdrop-blur-sm">
          <div className="max-w-[1280px] mx-auto px-4 sm:px-6 py-3 flex flex-wrap items-center justify-between gap-y-2 gap-x-4 sm:gap-x-6 text-xs text-ink-200 font-mono tabular-nums">
            <span><span className="text-ink-400">Today 24K</span> <span className="text-brand-200">{rates.g24}</span></span>
            <span><span className="text-ink-400">22K</span> <span className="text-brand-200">{rates.g22}</span></span>
            <span><span className="text-ink-400">18K</span> {rates.g18}</span>
            <span><span className="text-ink-400">Silver</span> {rates.silver}</span>
            <span className="hidden md:inline text-ink-400">Updated {rates.updatedAt}</span>
          </div>
        </div>
      </section>
        ...
        <div className="max-w-[1280px] mx-auto px-4 sm:px-6 py-14 sm:py-20 md:py-24 grid grid-cols-1 lg:grid-cols-[1.4fr_1fr] gap-6 md:gap-8">
          <Link to="/store/collections/bridal" className="group relative block aspect-[4/5] lg:aspect-auto lg:min-h-[560px] overflow-hidden bg-ink-100">
            ...
            <div className="absolute bottom-0 left-0 right-0 p-6 sm:p-8 md:p-10 text-ink-0">
              ...
              <p className="mt-2 sm:mt-3 text-xs sm:text-sm text-ink-100/85 max-w-md">Twelve heirloom pieces, photographed in our Gurugram workshop. Long-form story, no carousel.</p>
      ...
      <section className="bg-ink-900 text-ink-100">
        <div className="max-w-[1280px] mx-auto px-4 sm:px-6 py-14 sm:py-20 md:py-24 grid grid-cols-1 lg:grid-cols-[1fr_auto] gap-8 sm:gap-10 items-end">
          <figure className="max-w-2xl">
            ...
            <blockquote className="mt-4 sm:mt-5 font-display text-2xl sm:text-[28px] md:text-[36px] leading-[1.15] text-ink-0">
      ...
      {/* Visit-us CTA */}
      <section>
        <div className="max-w-[1280px] mx-auto px-4 sm:px-6 py-12 sm:py-16 md:py-20 flex flex-col md:flex-row items-start md:items-center justify-between gap-6 border-t border-ink-100">
